...
impl EventDecoder {
    ...
    pub fn decode_event(
        ...
        let event_def = self
            ...
            .ok_or_else(|| {
                SoltraceError::EventDecode(
                    format!("No event found with discriminator: {:02x?}", discriminator)
                )
            })?;
    ...
    /// more complex logic.
    fn decode_event_data(&self, _event_def: &IdlEventDefinition, data: &[u8]) -> Result<serde_json::Value> {
