...
updated_at: 2026-07-28T12:30:56Z
...
Blocker resolved 2026-07-28T12:30Z: soltrace-ipo0 completed. All three primitives now exist (onchain_idl.rs, insert_or_replace/remove, Arc<ArcSwap<IdlParser>> EventDecoder field). Implemented.

## Summary of Changes

New file `soltrace-live/src/idl_subscription.rs` implementing the dedicated
IDL subscription task per the bean spec.

**Public API:**

- `spawn_idl_subscription_task(programs, ws_url, commitment, shared_parser) -> JoinHandle<()>`

**Design:**

- Pure `handle_account_notification(program, data, &shared_parser)` is the
  unit-testable core; WS plumbing is thin (per the bean's TDD guidance).
- Dedicated `PubsubClient` per session (never shares logs_subscribe's conn).
- `SelectAll` multiplexes per-PDA streams, each tagged with its program Pubkey.
- `HashMap<Pubkey, UnsubscribeFn>` tracks live subscriptions for clean teardown.
- Reconnect loop mirrors `main.rs:504-557` (capped backoff: >10 attempts → 60s).
- When all PDAs become immutable/closed → task exits cleanly (no spin).
- WS disconnect → reconnect + re-subscribe to all PDAs.
- `lamports == 0` ⇒ closed → drop from parser, unsubscribe, warn.
- `mutable == false` ⇒ SetImmutable → unsubscribe, log info.
- Decode failure (`Err`/`None`) ⇒ retain last-known-good, do NOT swap.

**Tests (8):** SetData swap-in, Close removal, empty-data=close, garbage
retains-last-good, non-Direct no-swap, SetImmutable unsubscribe, wrong-program
no-swap, backoff cap.

**Deps added:** `solana-account-decoder-client-types` (UiAccountEncoding),
`spl-program-metadata-client` (Metadata mutable check).

**Module declared** in main.rs; wiring into the startup sequence is a separate
binary-integration task (function marked `#[allow(dead_code)]` until wired).
